...
<script type="text/javascript">
    ...
    { //当页面加载状态为完全结束时进入
    	var buff = document.getElementsByClassName("buffview");
    	var i=0;
		...
		{
			String alert=request.getParameter("deleerr");
			if(alert.equals("99"))
			{
			%>
				alert("##删除成功##");
...
<%
	...
	int iAction=0;
	if(request.getParameter("staffac")==null){}
	else
	{
		iAction =Integer.parseInt(request.getParameter("staffac"));
		switch(iAction)
		{
			case 102:
...
<div id="staffinfo">
	<div id="search">
		<span id="infotitle">员工基础信息表</span>
		<form action="?action=101&staffac=102" method="post">
			<ul>
	...
	<div id="scrollbar">
		<div id="tableposi">
		    <table width="1000" id="table" align="center" cellpadding="4" cellspacing="1" >
				...
				<c:forEach var="row" items="${result.rows}"><!--迭代输出每一条数据-->
					<tr>
					   ...
					   <td>
					   		<a href="index.jsp?action=103&id=<c:out value="${row.staffid}"/>"><img src="img/recordedit.gif" title="修改"/></a>
					   		&nbsp		<!-- ${row.staffid}返还赋予修改的ID-->
					   		<a href="index.jsp?action=104&id=<c:out value="${row.staffid}"/>"><img src="img/recorddelete.gif" title="删除"/></a>
					   </td> 
					</tr>
				</c:forEach>
			</table>
