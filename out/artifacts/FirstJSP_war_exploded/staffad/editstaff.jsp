...
<div id="editstaffitem">
	<form id="edit-form" action="check/updateGOGO.jsp?editid=<%=id%>" onsubmit="return prom()" method="post"><!--提交表单时调用prom弹窗函数-->
		...
		<div>
			...
			<span>性别</span>
			<input type="radio" class="checkbox1"name="staffsex" value="男" required/>
			<a id="text1">男</a>
			<input type="radio" class="checkbox2" name="staffsex"value="女" required"/>
			<a id="text2">女</a>
		</div>
		<div>
		...
		<div>
			<span>所属部门</span><input type="text"class="inputposi" name="deptname"  autocomplete="off" value="${row.deptname}" required/>
		</div>
		<div>
			<span>出生日期</span><input type="date"class="inputposi" name="staffborn"  value="${row.staffborn}" required/>
		</div>
		<div>
			<span>入职日期</span><input type="date" class="inputposi" name="staffentry" disabled="disabled" value="${row.staffentry}" required/>
		</div>
		<div>
			<span>户籍所在地</span><input type="text" class="inputposi" name="nativeplace"  value="${row.nativeplace}" required/>
		</div>
		<div>
			<span>现居住地</span><input type="text" class="inputposi" name="residence" autocomplete="off" value="${row.residence}" required/>
		</div>
		<div>
			<span>详细地址</span><input type="text" class="inputposi" name="address" autocomplete="off" value="${row.address}" required/>
		</div>
		<div>
			<span>月休息天数</span><input type="text" class="inputposi" name="Mrest" autocomplete="off" value="${row.Mrest}" required/>
		</div>
		<div>
			<span>基本薪资</span><input type="text" class="inputposi" name="salary" autocomplete="off" value="${row.salary}" required/>
		</div>
		<div>
			<span id="warm">
				<%
